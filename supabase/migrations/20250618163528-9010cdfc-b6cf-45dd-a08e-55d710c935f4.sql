...
-- First, let's make sure the pdf_submissions and extracted_materials tables exist
...
CREATE POLICY "pdf_submissions_update_policy"
  ON pdf_submissions
  FOR UPDATE
...
-- Allow users to update extracted materials for their studio (for approval/rejection)
CREATE POLICY "extracted_materials_update_policy"
  ON extracted_materials
  FOR UPDATE
  USING (studio_id = (SELECT studio_id FROM users WHERE id = auth.uid()));

-- Create updated_at trigger for both tables
CREATE OR REPLACE TRIGGER set_pdf_submissions_updated_at
  BEFORE UPDATE ON pdf_submissions
  FOR EACH ROW
  EXECUTE FUNCTION handle_updated_at();

CREATE OR REPLACE TRIGGER set_extracted_materials_updated_at
